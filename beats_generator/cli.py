...
@click.group()
def main() -> None:
    """Generate and analyze ambient/binaural sessions in WAV, FLAC, and MP3."""
...
@click.option(
    ...
    required=False,
    default=None,
    help="Generate from built-in preset.",
)
@click.option(
    "--reference",
    "reference_path",
    ...
    required=False,
    default=None,
    help="Reference audio file for style cloning.",
)
@click.option(
    "--profile",
    "profile_path",
    ...
    required=False,
    default=None,
    help="Existing profile JSON (from analyze-reference).",
)
@click.option(
    "--variant",
    type=click.Choice(list(list_variant_names())),
    default="close_match",
    show_default=True,
    help="Variant used for reference/profile generation.",
)
@click.option(
    "--name",
    type=str,
...
@click.option(
    ...
    type=int,
    default=None,
    help="Optional duration override in minutes.",
)
@click.option(
    "--formats",
    type=str,
    ...
    show_default=True,
    help="Comma-separated list: wav,flac,mp3",
)
...
@click.option("--mp3-bitrate-kbps", type=int, default=192, show_default=True)
@click.option(
    "--save-profile",
    type=click.Path(path_type=Path, file_okay=True, dir_okay=False),
    required=False,
    default=None,
    help="Optional path to save generated profile when using --reference.",
)
def create_command(
    preset: str | None,
    ...
    chunk_size: int,
    mp3_bitrate_kbps: int,
    save_profile: Path | None,
) -> None:
    selected_formats = _parse_formats(formats)
    ffmpeg_path = _require_ffmpeg_if_needed(selected_formats)
    output_dir.mkdir(parents=True, exist_ok=True)

    source_count = int(preset is not None) + int(reference_path is not None) + int(profile_path is not None)
    if source_count != 1:
        raise click.ClickException("Choose exactly one source: --preset, --reference, or --profile")

    wav_path = output_dir / f"{name}.wav"

    if preset is not None:
        session = get_preset_spec(preset)
        if duration_min is not None:
            session = _scale_session_duration(session, duration_min * 60)
        config = RenderConfig(sample_rate=sample_rate, output_subtype="PCM_24")
        render_session_to_wav(
            session=session,
            output_path=wav_path,
            log=click.echo,
        ...
        if profile_path is not None:
            ...
            profile = analyze_reference(input_path=reference_path)
            if save_profile is not None:
                save_reference_profile(profile=profile, output_path=save_profile)
                click.echo(f"profile saved: {save_profile}")
        duration_sec = (duration_min * 60) if duration_min is not None else 20 * 60
        generate_variant_from_profile(
            profile=profile,
    ...
    click.echo(
        "created: "
        + ", ".join(str(_with_extension(wav_path, file_format)) for file_format in selected_formats)
    )
...
@click.option(
    "--output-dir",
    type=click.Path(path_type=Path, file_okay=False, dir_okay=True),
    default=Path("assets/audio/reference"),
    show_default=True,
)
...
def analyze_command(
    ...
    if output_path is None:
        ...
        output_path = output_dir / f"{slug}_profile.json"
...
def _scale_session_duration(session: SessionSpec, duration_sec: int) -> SessionSpec:
    if duration_sec <= 0:
        raise click.ClickException("--duration-min must be positive")
    if duration_sec == session.duration_sec:
        return session

    scale = duration_sec / float(session.duration_sec)
    phases: list[PhaseSpec] = []
    for phase in session.phases:
        phases.append(
            PhaseSpec(
                start_sec=phase.start_sec * scale,
                end_sec=phase.end_sec * scale,
    ...
    return SessionSpec(
        ...
        duration_sec=duration_sec,
        carrier_left_hz=session.carrier_left_hz,
        carrier_right_hz=session.carrier_right_hz,
        phases=tuple(phases),
...
def _parse_formats(raw: str) -> tuple[str, ...]:
    ...
    for item in raw.split(","):
        ...
        if fmt not in allowed:
            ...
            raise click.ClickException(f"Unsupported format '{fmt}'. Use: {choices}")
        if fmt not in values:
            values.append(fmt)
    if not values:
        raise click.ClickException("No formats selected. Use at least one of wav,flac,mp3")
    return tuple(values)


def _require_ffmpeg_if_needed(formats: tuple[str, ...]) -> str | None:
    if all(fmt == "wav" for fmt in formats):
        return None
    ffmpeg_path = shutil.which("ffmpeg")
    if ffmpeg_path is None:
        raise click.ClickException("ffmpeg is required for flac/mp3 output formats")
    return ffmpeg_path


def _with_extension(wav_path: Path, file_format: str) -> Path:
    return wav_path.with_suffix(f".{file_format}")
...
def _write_additional_formats(
    ...
    if "flac" in formats:
        ...
        _encode_with_ffmpeg(
            ...
            input_path=wav_path,
            output_path=_with_extension(wav_path, "flac"),
        )
    if "mp3" in formats:
        assert ffmpeg_path is not None
        _encode_with_ffmpeg(
            ffmpeg_path=ffmpeg_path,
            args=["-c:a", "libmp3lame", "-b:a", f"{mp3_bitrate_kbps}k"],
            input_path=wav_path,
            output_path=_with_extension(wav_path, "mp3"),
        )
    if "wav" not in formats and wav_path.exists():
        wav_path.unlink()
