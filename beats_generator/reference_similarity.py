...
def list_variant_names() -> tuple[str, ...]:
    return tuple(spec.name for spec in _default_variant_specs())


def generate_variant_from_profile(
    profile: ReferenceProfile,
    variant_name: str,
    output_path: Path,
    duration_sec: int | None = None,
    ...
    effective_duration = duration_sec if duration_sec is not None else int(round(profile.duration_sec))
    variant = _variant_spec_by_name(variant_name)
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def _variant_spec_by_name(name: str) -> VariantSpec:
    for spec in _default_variant_specs():
        if spec.name == name:
            return spec
    choices = ", ".join(list_variant_names())
    raise ValueError(f"Unknown variant '{name}'. Choose one of: {choices}")
